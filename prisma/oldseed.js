...
async function main() {
  // Clear existing data
  await prisma.wishlistPlant.deleteMany();
  await prisma.ownedPlant.deleteMany();
  ...
  // Add some initial owned and wishlist plants
  await prisma.ownedPlant.create({ data: { plantId: plants[0].id } }); // Moonlight Serenade
  await prisma.ownedPlant.create({ data: { plantId: plants[1].id } }); // Berry Burst
  await prisma.ownedPlant.create({ data: { plantId: plants[4].id } }); // Coral Sunset
  await prisma.ownedPlant.create({ data: { plantId: plants[6].id } }); // Tiny Treasure
  await prisma.ownedPlant.create({ data: { plantId: plants[11].id } }); // Blueberry Muffin

  await prisma.wishlistPlant.create({ data: { plantId: plants[3].id } }); // Purple Majesty
  await prisma.wishlistPlant.create({ data: { plantId: plants[8].id } }); // Crimson Velvet
  await prisma.wishlistPlant.create({ data: { plantId: plants[14].id } }); // Starry Night

  console.log('Database seeded with 20 fictional African violets!');
}
